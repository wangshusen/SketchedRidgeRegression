...
# $SPARK_HOME/bin/spark-submit /Users/shusenwang/Documents/RidgeRegression/main/spark/modelAvgNpyFile.py --master local[7] 
...
def list2mat(listLabelVec, k):
    '''
    Input
        listLabelVec: List of (d+1)-dimensional Array,
            where the first element is the label y_i and the remaining elements are the entries of the data x_i
        k: integer
    Output
    ...
    # concatinate the (y_i, x_i) to form X and y
...
#             .setMaster("local[3]")
    ...
    rddLabelVectorTrain = sc.parallelize(matTrain, param['numPartitions']).cache()
    # read and parse the test data
    matTest = numpy.load(param['testFileName'])
    rddLabelVectorTest = sc.parallelize(matTrain, param['numPartitions']).cache()
